...
import 'package:xingshan/widget/pull/pull_load_widget.dart';
import 'nested_refresh.dart';

///通用下上刷新控件
class XSNestedPullLoadWidget extends StatefulWidget {
  ///item渲染
  ...
  ///控制器，比如数据和一些配置
  final XSPullLoadWidgetControl control;
  ...
  @override
  _XSNestedPullLoadWidgetState createState() => _XSNestedPullLoadWidgetState(
      this.control,
      this.itemBuilder,
      this.onRefresh,
      this.onLoadMore,
...
class _XSNestedPullLoadWidgetState extends State<XSNestedPullLoadWidget> {
  final IndexedWidgetBuilder itemBuilder;
  ...
  XSPullLoadWidgetControl control;

  _XSNestedPullLoadWidgetState(this.control, this.itemBuilder, this.onRefresh,
      ...
      child: new Column(
        ...
        children: <Widget>[
          FlatButton(
            onPressed: () {},
            child: new Image(
                image: new AssetImage(XSICons.DEFAULT_USER_ICON),
                width: 70.0,
                height: 70.0),
          ),
          Container(
            child: Text(CommonUtils.getLocale(context).app_empty,
                style: XSConstant.normalText),
          ),
        ],
      ),
    );
